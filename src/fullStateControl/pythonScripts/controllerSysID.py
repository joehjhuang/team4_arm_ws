...
def main():
    ...
    global tPub
    tPub = rospy.Publisher('tPub', Float32, queue_size = 10)
...
def callback(encData):
    ...
    # determine amount of torque being applied to SEA
    backlash = 0
    angularDeflection= abs(x[0,0]-x[1,0])- backlash
    if angularDeflection< 0:
        seaTorque = 0
    else: 
        seaTorque = angularDeflection*14
    tPub.publish(float(seaTorque))
